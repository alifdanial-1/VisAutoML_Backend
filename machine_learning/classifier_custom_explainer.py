...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('classifier_operations.log'),
...
def prepare_model(drop, IDColumn):
    try:
        logger.info(f"Starting model preparation with IDColumn: {IDColumn}")
        zero_label = None
        one_label = None
...
def flask_main(x_train, x_test, y_train, y_test, catCols, model_id, auto):
    try:
        logger.info(f"Starting flask_main with model_id: {model_id}, auto: {auto}")
...
if __name__ == '__main__':
    ...
    if zero_label != None:
        label0 = zero_label
        label1 = one_label
    explainer, CustomDashboard, CustomPredictionsTab, Classif = flask_main(x_train, x_test, y_train, y_test, catCols, model_id, auto)
